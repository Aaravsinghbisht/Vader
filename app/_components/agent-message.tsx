...
function AgentMessagePart({
  ...
  switch (part.type) {
    ...
    case "dynamic-tool": {
      ...
      const reportMarkdown = extractAreaReport(part);
        ...
        <>
          <Tool
            defaultOpen={
              ...
              (part.state !== "output-available" && reportMarkdown === null)
            }
          >
            <ToolHeader
              state={part.state}
            ...
            <ToolContent>
              ...
              {reportMarkdown === null ? (
                ...
                <p className="px-4 pb-3 text-muted-foreground text-sm">
                  Research complete — full briefing rendered below.
                </p>
              )}
            </ToolContent>
          </Tool>
          {reportMarkdown ? (
            <MessageResponse className="mt-2">{reportMarkdown}</MessageResponse>
          ) : null}
        </>
      );
    }
...
function extractAreaReport(part: EveDynamicToolPart): string | null {
  if (part.toolName !== "area_situation_report" || part.state !== "output-available") {
    return null;
  }
